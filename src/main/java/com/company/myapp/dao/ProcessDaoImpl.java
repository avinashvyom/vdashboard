...
import com.company.myapp.config.DataSourceConfiguration;
...
//	}
	...
	public void createMetaDataTables() throws Exception {
		...
		String Name = null;
		try {
			Name = this.getQueryResult("select name from SYS.tables where tables.name='ProcessMetaData'");
			...
			Name = this.getQueryResult("select name from SYS.tables where tables.name='FieldMetaData'");
	...
	public String getQueryResult(String sql) throws Exception {
		try {
			this.setDataSource();
			return this.jdbcTemplateObject.query(sql, new ResultSetExtractor<String>() {
	...
	@Transactional
	public void insertFieldMetaData(String ProcessName, String[] fieldNames, String[] Datatypes, String[] length)
			throws Exception {
		try {
			String insertProcessMetaDataQuery = "insert INTO" + " " + "ProcessMetaData" + " " + "(ProcessName)" + " "
					+ "Values" + "(" + "'" + ProcessName + "'" + ")";
			this.jdbcTemplateObject.execute(insertProcessMetaDataQuery);
			String getSQLQuery = "select ID from ProcessMetaData where ProcessName=" + "'" + ProcessName + "'";
			Integer id = this.jdbcTemplateObject.queryForObject(getSQLQuery, int.class);
			String sqlReqID = "INSERT INTO FieldMetaData VALUES(" + id + ",'ReqID','String',20)";
			this.jdbcTemplateObject.execute(sqlReqID);
			for (int i = 0; i <= (fieldNames.length - 1); i++) {
				String insertFieldMetaDataQuery = "INSERT INTO" + " " + "FieldMetaData" + " " + "VALUES" + "(" + "'"
						+ id + "'" + "," + "'" + fieldNames[i] + "'" + "," + "'" + Datatypes[i] + "'" + "," + "'"
						+ length[i] + "'" + ")";
				this.jdbcTemplateObject.execute(insertFieldMetaDataQuery);
			}
			String sqlStatus = "INSERT INTO FieldMetaData VALUES(" + id + ",'Status','String',20)";
	...
	@Transactional
	public void createProcessTable(String ProcessName, String[] fieldNames, String[] Datatypes, String[] length)
			throws Exception {
		try {
			String sqlQuery = this.buildSqlQuery(ProcessName, fieldNames, Datatypes, length);
	...
	@Transactional
	public void createAuditProcessTable(String ProcessName, String[] fieldNames, String[] Datatypes, String[] length)
			throws Exception {
		try {
	...
	public String createSqlTrigger(String ProcessName, String[] fieldNames, String[] Datatypes, String[] length)
			throws Exception {
		StringBuilder stringBuilder = null;
		try {

			String sql = "CREATE TRIGGER UPD_AUDIT_" + ProcessName + " ON " + ProcessName + " AFTER UPDATE \n "
					+ "AS \n " + "INSERT INTO AUDIT_" + ProcessName + " ( CRUD, UPDATE_TIME, ID, Status, Error_Msg ";

			stringBuilder = new StringBuilder(sql);
			for (int i = 0; i <= (fieldNames.length - 1); i++) {
				stringBuilder.append(", " + fieldNames[i]);
			}
			stringBuilder.append(") SELECT 'U',CURRENT_TIMESTAMP, ReqID, Status, Error_Msg");
	...
	public String buildSqlQuery(String ProcessName, String[] fieldNames, String[] Datatypes, String[] length)
			throws Exception {
		StringBuilder stringBuilder = null;
		try {
			...
			stringBuilder.append(",Status varchar(25), Error_Msg varchar(5000))");
			//System.out.println("SQL = " + stringBuilder.toString());
		} catch (Exception e) {
			// e.printStackTrace();
			throw e;
	...
	public String buildAuditSqlQuery(String ProcessName, String[] fieldNames, String[] Datatypes, String[] length)
			throws Exception {
		StringBuilder stringBuilder = null;
		try {
	...
	public Map<String, Object> viewProcessByID(Integer ID, String processName) throws Exception {
		String sql = null;
		Map<String, Object> processRecords = null;
		Map<String, String> namedParameters = new HashMap<String, String>();
		try {
			sql = "SELECT * FROM " + "\"" + processName + "\"" + " WHERE ReqID=" + ID;
			List<Map<String, Object>> rows = this.getJdbcTemplate().queryForList(sql, namedParameters);
	...
	public List<ProcessDto> listProcessData(String processName, String status) throws Exception {
		...
		try {
			if (status.equals("Both")) {
				...
				sql = "SELECT * FROM " + "\"" + processName + "\"" + " WHERE Status='" + status + "'";
			}
			Map<String, String> namedParameters = new HashMap<String, String>();
			List<Map<String, Object>> rows = this.getJdbcTemplate().queryForList(sql, namedParameters);
	...
	public void updateProcess(ProcessDto processDto, String pType) throws Exception {
		try {
			String updateSql = null;
			if (pType.equals("NAV")) {
				updateSql = "UPDATE NAV SET ReqID = :reqID, " + "AppNo = :applicationNo, " + "AreaCode = :areaCode, "
						+ "BankCode = :bankCode, " + "Premium = :premium, " + "ProductType = :producttype, "
						+ "PayType = :payType, " + "Status = :status " + "WHERE ReqID = :reqID";
			} else if (pType.equals("TermClaim")) {
				updateSql = "UPDATE Termclaim SET PolicyNumber = :policyNo, " + "Name = :name, "
						+ "Address = :address, " + "age = :age, " + "Premium = :premium, "
						+ "ProductType = :producttype, " + "PayType = :payType, " + "Status = :status "
						+ "WHERE PolicyNumber = :policyNo";
			}
			SqlParameterSource namParameterSource = new BeanPropertySqlParameterSource(processDto);
	...
	public List<Map<String, Object>> getProcessData(String processName, String status) throws Exception {
		List<Map<String, Object>> processData = null;
		String sql = "";
		try {
			if (status.equals("All")) {
				sql = "SELECT * FROM " + "\"" + processName + "\"" + " ORDER BY ReqID ASC";
			} else {
				sql = "SELECT * FROM " + "\"" + processName + "\"" + " WHERE Status='" + status + "'";
			}
			Map<String, String> namedParameters = new HashMap<String, String>();
			processData = this.getJdbcTemplate().queryForList(sql, namedParameters);
	...
	public List<String> getColumnNames(String processName) throws Exception {
		...
		try {
			this.setDataSource();
			String sql = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME ='" + "\"" + processName
					+ "\"" + "'";
			processCol = this.jdbcTemplateObject.queryForList(sql, String.class);
		} catch (Exception e) {
			// e.printStackTrace();
			throw e;
		}
	...
	public void updateProcess(MultiValueMap<String, String> requestParams) throws Exception {
		try {
			this.setDataSource();
	...
	public String buildUpdateSqlQuery(MultiValueMap<String, String> requestParams) throws Exception {
	...
	public List<Map<String, Object>> getProcessColumns(String processName) throws Exception {
		String sql = null;
		...
		try {
			sql = "select f.FieldName,f.DataType,f.length from FieldMetaData f "
					+ "inner join ProcessMetaData p ON p.ID = f.ID " + "where p.ProcessName = " + "'" + processName
					+ "'";
	...
	public String validateProcessName(String processName) throws Exception {
		try {
			String sql = "SELECT TOP 1 ProcessName FROM ProcessMetaData WHERE ProcessName = '" + "\"" + processName
					+ "\"" + "'";
	...
	// Prasad
	@Transactional
	public ModelAndView sendAuditDataByProcessName(String pType, UserDto userDto) throws SQLException {
		// TODO Auto-generated method stub
		ModelAndView mv = new ModelAndView("displayAuditData");
		JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
		//System.out.println(dataSource.getConnection().getSchema().toString());
	...
	private boolean checkIfTableExists(String auditTableName) {
		JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
	...
	public List<String> getAuditTablesList() {
		JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
		String sql = "SELECT * FROM INFORMATION_SCHEMA.TABLES where TABLE_NAME like '%Audit%';";
		List<Map<String, Object>> tableData = jdbcTemplate.queryForList(sql);
		//System.out.println(tableData.toString());
		List<String> auditTablesList = new ArrayList<String>();
		for (Map<String, Object> table : tableData) {
			String auditTableName = table.get("TABLE_NAME").toString();
			StringBuffer sb = new StringBuffer(auditTableName);
			sb.replace(0, 6, "");
			auditTableName = sb.toString();
			auditTablesList.add(auditTableName);
		}
		//System.out.println("Audit Tables List : " + auditTablesList.toString());
		return auditTablesList;
	}
}